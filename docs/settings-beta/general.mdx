...
When running Agentic Kanban on a remote server (e.g., accessed via Cloudflare tunnel, ngrok, or as a systemctl service), you can configure VSCode-based editors to open projects via SSH instead of assuming localhost.
...
- Agentic Kanban runs on a remote server (VPS, cloud instance, etc.)
...
Set a prefix for auto-generated branch names. When you create a new workspace or task, Agentic Kanban automatically creates a git branch for your changes.
...
Specify where Agentic Kanban stores workspace data. Workspaces are created in a `.agentic-kanban-workspaces` subdirectory within this path.
...
Configure how Agentic Kanban alerts you about task progress and status changes.

- **Sound Effects** - Play audio notifications when tasks complete, need attention, or encounter errors. Useful when working with multiple tasks or when Agentic Kanban runs in a background tab.
- **Push Notifications** - Receive browser notifications even when Agentic Kanban isn't in focus. Requires browser permission when first enabled.
...
Enable or disable telemetry data collection to help improve Agentic Kanban.
