...
<Frame>
<img src="/images/vk-settings.png" alt="Agentic Kanban global settings page showing theme options, agent configuration, and settings" />
</Frame>
...
<Frame>
<img src="/images/vk-editor-ssh.png" alt="Agentic Kanban settings editor section showing ssh configuration options." />
</Frame>
...
Telemetry is disabled by default. You can opt in to telemetry data collection to help improve Agentic Kanban.

Self-hosted or forked builds only send telemetry after you opt in and configure your own telemetry project settings. Backend PostHog uses the runtime `POSTHOG_API_KEY` and `POSTHOG_API_ENDPOINT` environment variables. Backend Sentry uses `VIBE_KANBAN_BACKEND_SENTRY_DSN` or `SENTRY_DSN`. Frontend telemetry uses `VITE_POSTHOG_API_KEY`, `VITE_POSTHOG_API_ENDPOINT`, and `VITE_SENTRY_DSN` at build time.
