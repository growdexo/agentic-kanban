...
async function main() {
      ...
      .then((latest) => {
        if (latest && latest !== CLI_VERSION) {
          setTimeout(() => {
            ...
            console.log(`Run: npx agentic-kanban@latest`);
          }, 2000);
        }
      })
      .catch(() => {});
  ...
  if (isMcpMode) {
    await extractAndRun("agentic-kanban-mcp", (bin) => {
    ...
    await extractAndRun("agentic-kanban-review", (bin) => {
    ...
    console.log(`Starting agentic-kanban v${CLI_VERSION}${modeLabel}...`);
    await extractAndRun("agentic-kanban", (bin) => {
      if (platform === "win32") {
