...
impl WorktreeManager {
    ...
    /// Get the base directory for agentic-kanban worktrees
    pub fn get_worktree_base_dir() -> std::path::PathBuf {
        if let Some(override_path) = WORKSPACE_DIR_OVERRIDE.get() {
            ...
            return override_path.join(".agentic-kanban-workspaces");
        }
        Self::get_default_worktree_base_dir()
    }
    ...
    pub fn get_default_worktree_base_dir() -> std::path::PathBuf {
        utils::path::get_agentic_kanban_temp_dir().join("worktrees")
    }
